...
/**
 * Gizlilik Politikası — YÜRÜRLÜKTEKİ METNİN TEK KAYNAĞI.
 *
 * Ayrıştırmanın gerekçesi `kullanim-sartlari.tsx` başlığında ayrıntılı yazılı;
 * özeti: metin iki yüzeyde görünür (`/gizlilik` ve `/panel/gizlilik`) ve iki
 * dosyada durursa biri güncellenip diğeri unutulur. Burada ek bir sebep daha
 * var: bölümlere metnin İÇİNDEN numarayla atıf yapılıyor ("7. bölüm",
 * "10. bölüm") ve o numaralar sunum sırasında dizi indisinden üretiliyor —
 * yani sunum da veri kadar paylaşılmak zorunda.
 *
 * `h1` bileşenin DIŞINDA — burada `h1` yok: genel sayfa kendi pazarlama
 * başlığını, panel `SayfaBasligi`yi çiziyor. Amaç sayfa başına tek `h1`
 * (frontend-contract §10.2 · S2).
 */
        ...
        <dl className="flex flex-col gap-4">
          {satirlar.map(([etiket, deger]) => (
            <div key={etiket} className="flex flex-col gap-1 md:flex-row md:gap-4">
              ...
              <dd className="max-w-[70ch] text-sm leading-relaxed text-muted">{deger}</dd>
            </div>
          ))}
        </dl>
      </Card>
      ...
      <ol className="flex flex-col gap-2">
        {HAKLAR.map((hak, i) => (
          <li key={i} className="flex gap-3">
            ...
            <p className="max-w-[70ch] text-sm leading-relaxed text-muted">{hak}</p>
          </li>
        ))}
      </ol>
    </Card>
...
export function GizlilikMetni({
  ...
  return (
    <div className={className}>
      <p className="text-sm text-muted">Son güncelleme: {SON_GUNCELLEME}</p>

      <p className="mt-6 max-w-[70ch] text-base leading-relaxed text-muted md:text-lg">
      ...
      <Alert tone="info" className="mt-6 max-w-[70ch]">
      ...
      <p className="mt-8 max-w-[70ch] text-sm leading-relaxed text-muted">
        ...
        <Link
          href={yasalYol(yuzey, 'kullanim-sartlari')}
